...
if [ "$(grep -Fc 'persist-credentials: false' "$workflow")" -lt 2 ]; then
  echo "semantic-review trusted checkouts should not persist write-capable credentials" >&2
  exit 1
fi
...
if grep -Fq 'ref: ${{ steps.pr.outputs.head_sha }}' "$workflow"; then
  echo "semantic-review must not checkout PR head in the trusted workflow" >&2
  exit 1
fi
