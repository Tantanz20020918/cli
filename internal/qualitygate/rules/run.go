...
func Run(ctx context.Context, opts Options) ([]report.Diagnostic, facts.Facts, error) {
	m, err := manifest.Collect(ctx)
	if err != nil {
		return nil, facts.Facts{}, err
	}
	commandIndex, err := manifest.CollectCommandIndex(ctx)
	if err != nil {
		return nil, facts.Facts{}, err
	}
...
func loadBaseReferenceManifest(ctx context.Context, repo, changedFrom string) (*manifest.Manifest, bool, error) {
	...
	for _, source := range []struct {
		...
		{path: "qualitygate/golden/command_index.golden.json", complete: true},
		{path: "qualitygate/golden/command_manifest.golden.json", complete: false},
	} {
