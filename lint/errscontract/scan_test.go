...
`,
		...
		"qualitygate/allowlists/legacy-command-errors.txt": "cmd/legacy.go\t" +
...
`,
		"qualitygate/allowlists/legacy-command-errors.txt": "cmd/clean.go\t7\tcli-owner\tlegacy command boundary bare error\t" + addedAt + "\n",
	})
	v, err := ScanRepoWithOptions(root, ScanOptions{})
	if err != nil {
		t.Fatalf("ScanRepoWithOptions: %v", err)
	...
	for _, vv := range v {
		if vv.Rule == "legacy_command_error_allowlist" &&
			...
			vv.File == "qualitygate/allowlists/legacy-command-errors.txt" &&
			vv.Line == 1 {
			return
		}
	}
